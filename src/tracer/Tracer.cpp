...
void Tracer::Insert(const std::string &function, uint32_t object_param, uint32_t esp) {
...
void Tracer::Remove(const std::string &function, uint32_t current_esp) {
    ...
    for (auto it = entries.begin(); it != entries.end(); ++it) {
        ...
        uint32_t diff = (current_esp > entry_esp)
                            ? (current_esp - entry_esp)
...
extern "C" void TracerPushEntry(const char *function, uint32_t object_param, uint32_t esp) {
    if (function) {
